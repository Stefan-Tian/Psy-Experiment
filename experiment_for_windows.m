...
DrawFormattedText(t1,double('接下來您會看到一系列由人工智慧系統評分外貌以及社經地位較高的照片。'),'center',350,256);
DrawFormattedText(t1,double('請您依據照片人像的外貌好感度及其可能的社經地位去做1-7的評分。'),'center', 375,256); 
DrawFormattedText(t1,double('7表示好感度最高及社經地位最高；'),'center', 400, 256); 
DrawFormattedText(t1,double('1表示好感度最低及社經地位最低。'),'center', 425,256); 
DrawFormattedText(t1,double('每張照片將會呈現3秒，之後每題有兩秒的作答時間'),'center', 450,256); 
DrawFormattedText(t1,double('作答時請按鍵盤上方的1-7數字'),'center', 475,256); 
DrawFormattedText(t1,double('請點擊螢幕以繼續'),'center',500,256); 
...
t1= Screen('MakeTexture',w, image);
Screen('TextFont',t1,'Kaiti'); 
DrawFormattedText(t1,double('接下來將進行十次練習次'),'center',400,256);
DrawFormattedText(t1,double('用滑鼠點擊螢幕以繼續'),'center',450,256); 
Screen('DrawTexture',w, t1);
Screen('Flip',w);
GetClicks;
...
for g = 1:2
    ...
    for n = 1:5
        ...
        for q = 1:2
           if (q == 1)
               question = double('現在請針對剛才圖片中人物的外貌進行1-7評分（請按鍵盤右側數字鍵）');
           else
               question = double('現在請針對剛才圖片中人物的社經地位進行1-7評分（請按鍵盤右側數字鍵）');
           end
